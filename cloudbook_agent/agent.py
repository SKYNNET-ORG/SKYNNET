...
def get_project_agent_id():
	...
	print(my_project_folder + " - " + my_agent_ID)
...
def stop():
	...
	if platform.system()=="Windows":
		...
		# proc.kill()
		kill_tree_command = "TASKKILL /F /T /PID "+str(actual)
		...
		try:
			os.killpg(os.getpgid(actual), signal.SIGTERM)
		except Exception as e:
			if not verbose:
				builtins.print(e)
	
	#+' '+str(actual[1])+' '+str(actual[2])+' '#+str(threading.current_thread())
	#return 'hola'
"""
################################################################################
		...
		while not mp_flask2agent_queue.empty():
			...
			if "flask_proc_ok" in item:
				...
				try:
					...
					flask_proc.start()
					añadirJson(my_agent_ID, flask_proc.pid,fs_path+ os.sep )
