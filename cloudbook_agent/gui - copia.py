...
#prueba
import psutil  # Para obtener el uso de la CPU en tiempo real
...
class AgentXTab(ttk.Frame):
	...
	def actualizar_grafico(self):
		...
		self.valores = [uso_cpu, 100 - uso_cpu]
		self.etiquetas = ['Uso de CPU', 'Sin uso']
		self.colores = ['lightcoral', 'lightgrey']
		self.ax.clear()
		self.ax.pie(self.valores, labels=self.etiquetas, colors=self.colores, autopct='%1.1f%%', startangle=90)
		self.ax.axis('equal')  # Hacer que el gráfico sea un círculo
		self.canvas.draw()
		# Programar la próxima actualización
		#root.after(1000, self.actualizar_grafico)
	...
	def __init__(self, *args, agent_info, project_name):
		...
		self.ax.pie([100], labels=['Uso de CPU'], colors=['lightgrey'], autopct='%1.1f%%', startangle=90)
		self.ax.axis('equal')
...
class ProjectTab(ttk.Frame):
	def __init__(self, *args, project_name):
		...
		print("self.tabs")
	def pestaña(self):
		self.focused_tab = self.notebook.tab(self.notebook.select(), "text")
...
class Application(ttk.Frame):
	...
	def grafo(self):
		try:
			...
			focused_tab_agente=globals()["ProjectTab_" + focused_tab].pestaña()[-1]

			agente=globals()["ProjectTab_" + focused_tab].tabs["AgentXTab"][int(focused_tab_agente)]
			agente.actualizar_grafico()
			print('aaa')
		except Exception as e:
			pass
		root.after(1000, self.grafo)
	...
	def refresh(self):
		...
		if focused_tab is not None:
			try:
				...
				pass
		#pestaña=globals()["ProjectTab_" + focused_tab].pestaña()
		#globals()["ProjectTab_" + focused_tab]
		#plot=globals()["ProjectTab_" + focused_tab]
...
if __name__ == '__main__':
	...
	t = threading.Thread(target=app.grafo())
	t.start()
